...
def classify_morphology_path(
    ...
    #
    # المشكلة السابقة: كانت لواحق كـ ونَ (المذكر الجمع الاسمي) تُغطّي الأفعال
    # المضارعة الجمعية (تُدِيرُونَ، تَكُونُونَ، ...) بسبب تطابق اللاحقة ونَ،
    # فكانت هذه الأفعال تُصنَّف NOMINAL_MORPHOLOGY_PATH ظلمًا.
    #
    # الحل: _is_mudaric_surface تحتوي بالفعل على حارس حرف المد (surface[2] not in
    # _LONG_VOWEL_LETTERS) الذي يُميّز بين المضارع الحقيقي (نَذْهَبُ: نَ+ذ) والمشتق
    # الاسمي (نَاءِمِينَ: نَ+ا → False). لذلك تقديم المضارع أولًا آمن تمامًا:
    # نَاءِمِينَ: _is_mudaric = False → يستمر إلى فحص اللاحقة.
    # تُدِيرُونَ: _is_mudaric = True  → VERBAL_ROOT_PATH مباشرة.
